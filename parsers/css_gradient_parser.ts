...
// ---------------------------------------------------------------
//  Type definitions for parsed gradient data
// ---------------------------------------------------------------
 ...
 */
function splitGradientArgs(argsStr: string): string[] {
    const result: string[] = [];
    let current = '';
    let depth = 0;
 ...
 */
function extractFunctionArgs(value: string): string {
...
const DIRECTION_KEYWORD_MAP: Record<string, number> = {
    'to top':          0,
    'to top right':    45,
    'to right top':    45,
    'to right':        90,
 ...
 */
function parseLinearDirection(token: string): number | null {
 ...
 */
function parseColorStop(token: string): { color: { r: number; g: number; b: number; a: number }; position: number | null } | null {
    ...
    try {
        color = parseToLinearColor(colorPart);
    } catch {
        return null;
    }
    ...
    if (!color || (color.r === 0 && color.g === 0 && color.b === 0 && color.a === 0 && !isBlackish(colorPart))) {
        ...
        try {
            color = parseToLinearColor(trimmed);
            positionPart = null;
        } catch {
            return null;
        }
...
function isBlackish(colorStr: string): boolean {
 ...
 */
function normalizeStopPositions(stops: { color: { r: number; g: number; b: number; a: number }; position: number | null }[]): GradientColorStop[] {
 ...
 */
export function parseLinearGradient(value: string): LinearGradientData | null {
    ...
    const rawStops: { color: { r: number; g: number; b: number; a: number }; position: number | null }[] = [];
 ...
 */
export function parseRadialGradient(value: string): RadialGradientData | null {
    const args = extractFunctionArgs(value);
    if (!args) return null;
 ...
 */
export function parseConicGradient(value: string): ConicGradientData | null {
    const args = extractFunctionArgs(value);
    if (!args) return null;
    ...
    if (fromMatch) {
        ...
        switch (fromMatch[2]) {
            case 'deg':  angleDegrees = val; break;
            case 'grad': angleDegrees = val * (360 / 400); break;
            case 'rad':  angleDegrees = val * (180 / Math.PI); break;
            case 'turn': angleDegrees = val * 360; break;
        }
        colorStopStartIndex = 1;
    }
 ...
 */
export function parseGradient(value: string): GradientData | null {
 ...
 */
export function createGradientBrush(
    ...
    let texture: UE.Texture2D | null = null;

    switch (gradient.type) {
        case 'linear':
            texture = UE.UMGManager.CreateLinearGradientTexture(
                context, gradient.angleDegrees, colors, positions, width, height
            );
            break;
        case 'radial':
            texture = UE.UMGManager.CreateRadialGradientTexture(
                context, colors, positions, width, height
            );
            break;
        case 'conic':

 ...
 */
export function isKnownCSSProperty(property: string): boolean {
    if (!property) return false;
    if (SUPPORTED_PROPERTIES.has(property)) return true;
    if (NOOP_PROPERTIES.has(property)) return true;
    if (VENDOR_PREFIX_REGEX.test(property)) return true;
 ...
 */
export function isNoopCSSProperty(property: string): boolean {
    if (!property) return false;
    if (NOOP_PROPERTIES.has(property)) return true;
    if (VENDOR_PREFIX_REGEX.test(property) && !SUPPORTED_PROPERTIES.has(property)) return true;
    return false;
}

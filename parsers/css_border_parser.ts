...
export interface BorderOutlineResult {
    hasOutline: boolean;
    width: number;
    cornerRadii: ParsedBorderRadius;
    color: { r: number; g: number; b: number; a: number } | null;
    drawAsRoundedBox: boolean;
}
 ...
 */
export function parseBorderShorthand(border: string, style?: any): ParsedBorder {
    const result: ParsedBorder = {
        width: 0,
        style: 'none',
        color: null
    };
    ...
    for (const token of tokens) {
        ...
        if (/^[\d.]/.test(token) || token === 'thin' || token === 'medium' || token === 'thick') {
            result.width = convertLengthUnitToSlateUnit(token, style);
            continue;
        }

        // Remaining token is treated as a color
        if (isColorToken(token)) {
            result.color = parseToLinearColor(token);
        }
    }
 ...
 */
export function parseBorderRadius(borderRadius: string | number, style?: any): ParsedBorderRadius {
    const result: ParsedBorderRadius = {
        topLeft: 0,
        topRight: 0,
        bottomRight: 0,
        bottomLeft: 0
    ...
    const values = primary.split(/\s+/).map(v => convertLengthUnitToSlateUnit(v, style));
 ...
 */
export function parseSingleCornerRadius(value: string | number, style?: any): number {
    if (typeof value === 'number') return value;
    if (!value || typeof value !== 'string') return 0;

    const parts = value.trim().split(/\s+/);
    return convertLengthUnitToSlateUnit(parts[0], style);
}
 ...
 */
export function parseBoxShadow(boxShadow: string, style?: any): ParsedBoxShadow | null {
 ...
 */
function parseSingleShadowLayer(layer: string, style?: any): ParsedBoxShadow | null {
    ...
    for (const token of tokens) {
        ...
        if (/^-?[\d.]/.test(token)) {
            lengthValues.push(convertLengthUnitToSlateUnit(token, style));
    ...
    if (lengthValues.length >= 1) result.offsetX = lengthValues[0];
    if (lengthValues.length >= 2) result.offsetY = lengthValues[1];
    if (lengthValues.length >= 3) result.blurRadius = lengthValues[2];
    if (lengthValues.length >= 4) result.spreadRadius = lengthValues[3];

    if (colorStr) {
        result.color = parseToLinearColor(colorStr);
    }
 ...
 */
export function processBorderStyles(style: any): BorderOutlineResult {
    const result: BorderOutlineResult = {
        hasOutline: false,
        width: 0,
    ...
    if (style.border) {
        ...
        if (parsed.width > 0) result.hasOutline = true;
    }

    // 2. Individual border-width override
    if (style.borderWidth !== undefined) {
        result.width = convertLengthUnitToSlateUnit(style.borderWidth, style);
        if (result.width > 0) result.hasOutline = true;
    }

    // 3. Individual border-color override
    if (style.borderColor) {
        result.color = parseToLinearColor(style.borderColor);
 ...
 */
export function applyOutlineToBrush(brush: UE.SlateBrush, outline: BorderOutlineResult): void {
    if (!brush || !outline) {
        return;
    }

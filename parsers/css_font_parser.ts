 ...
 */
export function parseTextShadow(textShadow: string, style?: any): {
    offsetX: number;
    offsetY: number;
    blurRadius: number;
    ...
    for (const token of tokens) {
        ...
        if (/^-?[\d.]/.test(token)) {
            lengths.push(convertLengthUnitToSlateUnit(token, style));
            continue;
        }
    ...
    return {
        ...
        offsetY,
        blurRadius,
        color: colorStr ? parseToLinearColor(colorStr) : null
    };
}

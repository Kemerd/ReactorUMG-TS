...
const pendingWidgets: Set<UE.Widget> = new Set();

/** Panel slots that need SynchronizeProperties called before next paint */
const pendingSlots: Set<UE.PanelSlot> = new Set();
 ...
 */
export function queueWidgetSync(widget: UE.Widget): void {
    if (!widget) return;
 ...
 */
export function queueSlotSync(slot: UE.PanelSlot): void {
    if (!slot) return;
 ...
 */
export function immediateWidgetSync(widget: UE.Widget): void {
    if (!widget) return;
 ...
 */
export function getSyncStats(): { queued: number; flushed: number; savings: number } {
 ...
 */
export function getPendingCount(): { widgets: number; slots: number } {
    return {

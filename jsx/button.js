...
class ButtonConverter extends jsx_converter_1.JSXConverter {
    ...
    setButtonTextColor(button, style, props) {
    ...
    applyStateBrush(button, state, source, fallbackBrush, fallbackTint) {
        let brush = null;
        if (source) {
            try {
                ...
                if (this.isSlateBrush(source)) {
                    ...
                    // fix: should convert source to ImageStyle
                    brush = (0, brush_parser_1.parseBrush)(source);
                }
            }
            catch (error) {
    ...
    setButtonBackground(button, style, props) {
        ...
        if (parsedBackground?.color) {
            ...
            const color = (0, css_color_parser_1.parseToLinearColor)(props.backgroundColor);
            button.BackgroundColor = new UE.LinearColor(color.r, color.g, color.b, color.a);
        }
        const hoverPseudoStyle = (0, cssstyle_parser_1.getAllStyles)(this.typeName, this.props, 'hover');
        const pressPseudoStyle = (0, cssstyle_parser_1.getAllStyles)(this.typeName, this.props, 'press');
        const disabledPseudoStyle = (0, cssstyle_parser_1.getAllStyles)(this.typeName, this.props, 'disabled');
        const parsedHoverPseudoBackground = (0, css_background_parser_1.parseBackgroundProps)(hoverPseudoStyle);
        const parsedPressPseudoBackground = (0, css_background_parser_1.parseBackgroundProps)(pressPseudoStyle);
        const parsedDisabledPseudoBackground = (0, css_background_parser_1.parseBackgroundProps)(disabledPseudoStyle);
    ...
    resolveProps(overrides) {
        ...
        const style = (0, cssstyle_parser_1.getAllStyles)(this.typeName, mergedProps);
        return { props: mergedProps, style };
    }
    applyEnabledState(button, props) {
    ...
    setButtonPadding(button, props, style) {
        const normalPaddingValue = this.toPaddingValue(props?.normalPadding ?? style?.normalPadding);
        if (normalPaddingValue) {
            const margin = (0, css_margin_parser_1.convertToUEMargin)(style || {}, normalPaddingValue, '', '', '', '');
            if (margin) {
        ...
        if (pressedPaddingValue) {
            const margin = (0, css_margin_parser_1.convertToUEMargin)(style || {}, pressedPaddingValue, '', '', '', '');
            if (margin) {
                button.WidgetStyle.PressedPadding = margin;
            }
        }
    }
    setInteractionMethods(button, props) {
        const clickMethod = props?.clickMethod;
        if (clickMethod) {
            const clickMethodMap = {
    ...
    initSingleEventHandler(button, propName, handler) {
    ...
    initButtonDefaultProps(button) {
        if (!button)
            return;
        // init image tint color
        function InitTintColorToWhite(tintColor) {
            if (!tintColor) {
    ...
    removeSingleEventHandler(button, propName) {
        const eventName = this.eventNameMapping[propName];
        if (!eventName) {
            return;
        }
    ...
    setButtonEventHandlers(button, props) {
        if (!props) {
            return;
        }

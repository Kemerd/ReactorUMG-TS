 ...
 */
class AudioConverter extends jsx_converter_1.JSXConverter {
    /* ------------------------------------------------------------------ */
    /*  Internal state                                                     */
    /* ------------------------------------------------------------------ */
    mediaPlayer;
        ...
        if (typeof props?.onEnded === 'function') {
            this.onEndedCallback = () => {
                try {
                    ...
                    console.warn('audio onEnded error:', e);
                }
            };
        ...
        if (typeof props?.onLoadedData === 'function' || typeof props?.onCanPlay === 'function') {
            this.onOpenedCallback = (_url) => {
                try {
                    ...
                    console.warn('audio onLoadedData error:', e);
                }
            };
        ...
        if (typeof props?.onError === 'function') {
            this.onOpenFailedCallback = (failedUrl) => {
                try {
                    ...
                    console.warn('audio onError error:', e);
                }
            };
        ...
        if (typeof props?.onPlay === 'function') {
            this.onPlaybackResumedCallback = () => {
                try {
                    ...
                    console.warn('audio onPlay error:', e);
                }
            };
        ...
        if (typeof props?.onPause === 'function') {
            this.onPlaybackSuspendedCallback = () => {
                try {
                    ...
                    console.warn('audio onPause error:', e);
                }
            };
    ...
    update(_widget, _oldProps, changedProps) {
        if (!this.mediaPlayer)
            return;
        // Source change
